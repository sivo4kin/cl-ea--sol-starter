...
type TransactSession struct {
	TransactOpts *TransactOpts // Ethereum account to send the transaction from
	CallOpts     *CallOpts     // Nonce to use for the transaction execution (nil = use pending state)
	*sync.Mutex
...
func (txSession *TransactSession) WaitTransaction(tx *types.Transaction) (*types.Receipt, error) {
	...
	var err error
	for {
		receipt, err = txSession.Backend.TransactionReceipt(context.Background(), tx.Hash())
